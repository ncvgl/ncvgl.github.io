---
layout: simple-post
title: "Coding is AI Agent Management"
date: 2026-03-02
...
I posted about building a Slack clone using multiple AI agents. Nobody asked "what did you build?". They asked: **"How did the agents talk to each other?"**

I started treating Claude like a smart assistant — write prompt, get code, review, repeat. That lasted about four hours. After 20-30 exchanges, context bloat kicked in: slower responses, confusion, compaction pauses.
...
To validate UI accuracy, I gave it two URLs: my local app and an actual Slack workspace I'm part of. Its job was to test the same features in both and flag differences. It clicks buttons, fills forms, pulls JavaScript from the page for advanced debugging, takes screenshots when things break. ([Chrome extension](https://code.claude.com/docs/en/chrome))
...
The agent had no way to know that. It needed me to step in and say: skip this one, it's not yours.
...
The key to speed isn't better prompts. It's removing yourself from the loop.
...
Writing functions, wiring up APIs, rendering UI — that's solved. Claude can do it with 90%+ accuracy if you give it clear requirements and a way to test its own work.
...
The goal isn't 10x speed. It's building a system that doesn't need you — and knowing exactly when to step back in.
